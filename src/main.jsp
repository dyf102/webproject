   ...
   <div class="page-container">
	...
	<%
		...
		Connection m_con = null;
            	...
            	String sql = "SELECT i.PHOTO_ID FROM (SELECT * FROM(SELECT DISTINCT COUNT_NUM FROM POPULARITY  ORDER BY COUNT_NUM DESC) WHERE ROWNUM <=5)num,(SELECT * FROM IMAGES im WHERE im.PERMITTED<>1 OR im.PERMITTED IN (SELECT GROUP_id FROM GROUP_LISTS WHERE FRIEND_ID='"+username+"'))i,POPULARITY p WHERE i.PHOTO_ID = p.PHOTO_ID AND p.COUNT_NUM = num.COUNT_NUM order by p.COUNT_NUM DESC";
            	String sql2 = "SELECT i.SUBJECT FROM (SELECT * FROM(SELECT DISTINCT COUNT_NUM FROM POPULARITY  ORDER BY COUNT_NUM DESC) WHERE ROWNUM <=5)num,(SELECT * FROM IMAGES im WHERE im.PERMITTED<>1 OR im.PERMITTED IN (SELECT GROUP_id FROM GROUP_LISTS WHERE FRIEND_ID='"+username+"'))i,POPULARITY p WHERE i.PHOTO_ID = p.PHOTO_ID AND p.COUNT_NUM = num.COUNT_NUM order by p.COUNT_NUM DESC";
            	try {
